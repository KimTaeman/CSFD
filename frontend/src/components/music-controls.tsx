...
const MusicControls = React.memo(() => {
  ...
  const announcedVolumePercentage = useMemo(
    () => Math.round(announcedVolume * 100),
    [announcedVolume],
  );
  ...
  const volumeAriaLabel = useMemo(() => `Music volume, ${volumePercentage}%`, [volumePercentage]);
  ...
  return (
    ...
    >
      ...
      <div className="sr-only">
        Music controls: Press Space or Enter to toggle playback. Use Tab to navigate to volume
        slider, then arrow keys to adjust volume.
      </div>
    </div>
  );
});
